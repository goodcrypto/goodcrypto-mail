...
from goodcrypto.mail.utils import email_in_domain, get_sysadmin_email, is_metadata_address
...
class Pipe(object):
    ...
    def process(self):
        ...
        def bounce_message(error_message):
            ...
            subject = i18n('{} - Undelivered Mail: Unable to encrypt message'.format(TAG_ERROR))
        ...
        if self.possibly_needs_encryption():
            ...
            try:
                ...
                if is_metadata_address(self.sender):
                    ...
                    bounce_message(i18n('Message originating from your metadata address'))
                    self.out_message = None
                else:
                    encrypt = Encrypt(crypto_message)
                    encrypted_message = encrypt.make_message_unreadable()
            ...
            try:
                ...
                debundled_message = debundle.make_messages_readable()
                ...
                drop_message(message_exception)
                self.out_message = None
            except Exception as exception:
                drop_message(exception)
                self.out_message = None
            except IOError as ioerror:
                drop_message(ioerror)
                self.out_message = None
    ...
    def reinject_message(self, message=None):
        ...
        try:
            if message is None:
                ...
                result_ok = utils.send_message(self.sender, self.recipient, message)

...
def add_multientry_header(message, header_name, multiline_value):
    '''
        ...
        @param  multiline_value         Multiline string
        @return   count of headers added
    '''
...
def is_multipart_message(message):
    '''
        ...
        >>> filename = get_plain_message_name('mime-multipart-text-html.txt')
        >>> with open(filename) as input_file:
        ...     email_message = EmailMessage(input_file)
        ...     is_multipart_message(email_message.get_message())
        True
    '''
...
def is_content_type_mime(message):
    '''
        ...
        >>> message = MIMEBase(mime_constants.APPLICATION_TYPE, mime_constants.OCTET_STREAM_SUB_TYPE)
        >>> is_content_type_mime(message)
        False
...
def get_user_id_matching_email(address, user_ids):
    '''
        ...
        True
        >>> get_user_id_matching_email(EDWARD_LOCAL_USER_ADDR, test_addresses) == EDWARD_LOCAL_USER_ADDR
        True
        >>> get_user_id_matching_email('test6@goodcrypto.local', test_addresses) is None
        True
        
        >>> from goodcrypto.oce import constants as oce_constants
        >>> test_addresses = [oce_constants.EDWARD_LOCAL_USER_ADDR, oce_constants.JOSEPH_REMOTE_USER_ADDR]
        >>> get_user_id_matching_email(EDWARD_LOCAL_USER, test_addresses) == EDWARD_LOCAL_USER_ADDR
        True
        >>> get_user_id_matching_email(EDWARD_LOCAL_USER_ADDR, test_addresses) == EDWARD_LOCAL_USER_ADDR
        True
...
def emails_equal(address1, address2):
    '''
        ...
        >>> # In honor of First Sergeant Menahem, who publicly denounced and refused to serve in operations 
        >>> # involving the occupied Palestinian territories because of the widespread surveillance of 
        >>> # innocent residents.
        >>> emails_equal('Menahem <menahem@goodcrypto.local>', 'menahem@goodcrypto.local')
        True
        >>> emails_equal('menahem@goodcrypto.local', 'menahem@goodcrypto.local')
        True
        >>> emails_equal('Menahem <menahem@goodcrypto.local>', 'Menahem@goodcrypto.local')
        True
        >>> emails_equal('menahems@goodcrypto.local', 'menahem@goodcrypto.local')
        False
    '''
...
def alt_to_text_message(original_message):
    ...
    try:
        ...
        charset = EmailMessage.get_charset(original_message)
        ...
        if content_type == mime_constants.MULTIPART_ALT_TYPE:
            ...
            for part in original_message.walk():
                if part.get_content_type() == mime_constants.TEXT_PLAIN_TYPE:
                    ...
                    charset = EmailMessage.get_charset(part)
            ...
            if text_message != None and DEBUGGING:
...
def get_encryption_software(email):
    ''' 
        ...
        @param  email        email, with or without the user name
        @return           encryption program list, or any empty list.
    '''
